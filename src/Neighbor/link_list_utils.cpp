...
void neighbor::create_grid(double xfixed, double hG, double xGmin, double xGmax, 
...
void neighbor::find_cell(double xPi, double yPi, int nGx, int nGy, 
...
void neighbor::find_neighborcells(int i_cell, int j_cell, double neighbor_scale, int nGx, int nGy,
    int &lx_nbc, int &rx_nbc, int &ly_nbc, int &ry_nbc)
{
      // ---define the west (x_loc -2) and east (x_loc+3)
    lx_nbc = i_cell - std::ceil(neighbor_scale);
    rx_nbc = i_cell + std::ceil(neighbor_scale);
    // ---define the south (y_loc -2) and north (y_loc+3)
    ly_nbc = j_cell - std::ceil(neighbor_scale);
    ry_nbc = j_cell + std::ceil(neighbor_scale);
