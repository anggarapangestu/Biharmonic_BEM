...
std::vector<std::vector<int>> neighbor::link_list(int np, const std::vector<double> &xp, const std::vector<double> &yp, double neighbor_scale)
{
    // ----------------------------------------------------------------------
    ...
    grid_space = Par::spc; // grid spacing
    ...
    this->create_grid(0.e0, grid_space, xgmin[0], xgmax[0], ncgrid[0], ngx_ll, xgrid);
    this->create_grid(0.e0, grid_space, xgmin[1], xgmax[1], ncgrid[1], ngy_ll, ygrid);
    ...
    for (int i = 0; i < np; i++)
    {
        this->find_cell(xp[i], yp[i], ngx_ll, ngy_ll, xgrid, ygrid, grid_space, i_c[i], j_c[i]);
    ...
    for (int i = 0; i < np; i++)
    {
        ...
        this->find_neighborcells(i_c[i], j_c[i], neighbor_scale, ngx_ll, ngy_ll, minxcell[0], maxxcell[0], minxcell[1], maxxcell[1]);
        //      Search grid:
        for (int ycell = minxcell[1]; ycell <= maxxcell[1]; ycell++)
        {
            for (int xcell = minxcell[0]; xcell <= maxxcell[0]; xcell++)
            {
                ...
                while (j > i)
                {
                    ...
                    msp = Par::spc; // (sp[i] + sp[j]) / 2.0e0;
                    if (_dr < ((neighbor_scale + 0.2) * msp))
                    {
