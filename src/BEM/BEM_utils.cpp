...
double calcBEM::calc_Bij(double a_ij, double k_ij, double L){
    ...
    Bij = 1/(2.0*M_PI) * 
    ( 
        (k_ij + L/2.0)*var1 - (k_ij - L/2.0)*var2 + _tan - L 
    );
    return Bij;
}
double calcBEM::calc_Cij(double a_ij, double k_ij, double L){
    // Internal variable declaration
    double Cij, var1, var2, _tan;
    ...
    Cij = a_ij/(8.0*M_PI) * 
    ( 
        2.0*(k_ij + L/2.0)*var1 - 2.0*(k_ij - L/2.0)*var2 + 2.0*_tan - (3.0/2.0)*L 
    );
    return Cij;
}
double calcBEM::calc_Dij(double a_ij, double k_ij, double L){
    ...
    Dij = 1/(8.0*M_PI) * 
    ( 
        (std::pow(a_ij, 2)*par1 + (1/3)*std::pow(par1, 3))*var1 - 
        (std::pow(a_ij, 2)*par2 + (1/3)*std::pow(par2, 3))*var2 - 
        (4.0/9.0) * (std::pow(par1, 3) - std::pow(par2, 3)) -
        (2.0/3.0) * std::pow(a_ij, 3) * _tan - (5.0/3.0) * std::pow(a_ij, 2) * L
    );
    return Dij;
}
...
double calcBEM::calc_a(double x_0, double y_0, double x_m, double y_m, double x_n, double y_n){
    // Calculation of a, the projection length of distance x_0 to x_m
    double a;
    a = (x_m - x_0) * x_n + (y_m - y_0) * y_n;
    return a;
}
double calcBEM::calc_k(double x_0, double y_0, double x_m, double y_m, double x_n, double y_n){
    // Calculation of k, the length from nearest point intersection to x_m
    double k;
